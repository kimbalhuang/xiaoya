...
// 音频优先级（默认 true=以数字人视频音轨为准，不重复播放；false=以原始 TTS 为准、视频静音）。
// 作为后端默认值下发给前端，用户可在设置页本地覆盖（localStorage，无需重启）。
const FLASHHEAD_AUDIO_SYNC = (process.env.FLASHHEAD_AUDIO_SYNC || 'true').toLowerCase() !== 'false';
...
wssBridge.on('connection', (ws) => {
  ...
  browserWs = ws;
  // 下发音频优先级默认值（前端可本地覆盖）；晚于启动连入的浏览器也能拿到初始配置
  ws.send(JSON.stringify({ type: 'config', audioSyncPreferVideo: FLASHHEAD_AUDIO_SYNC }));
  if (!bridge) {
...
async function bootstrap() {
  ...
  // 下发音频优先级默认值（前端可本地覆盖）；true=以数字人视频为准（防重复），
  // false=以原始 TTS 为准（视频静音，避免双声）。通过 /ws 通道（所有浏览器连接均接收）。
  sendToBrowser({ type: 'config', audioSyncPreferVideo: FLASHHEAD_AUDIO_SYNC });
