...
class FlashHeadAdapter {
  constructor({ mode, url, apiKey, onFrame, onMode, sendControl }) {
    ...
    this.portrait = null;
    this.ws = null;
    // 是否真正消费音频帧（Local/Gradio 需要 PCM 喂推理；TwoD 不需要）。
    // bridge 据此跳过无谓的 Opus→PCM 解码，避免实时语音(2D)模式下每帧空转浪费 CPU。
    this.consumesAudio = false;
  }
  init(portrait) {
...
class LocalAdapter extends FlashHeadAdapter {
  init(portrait) {
    super.init(portrait);
    this.consumesAudio = true; // 逐帧喂 PCM 给本地推理服务
    this.onMode?.('live');
    try {
...
class GradioAdapter extends FlashHeadAdapter {
  constructor(opts) {
    ...
    this.audioBuffer = [];
    this.consumesAudio = true; // Gradio 流式推理需要完整 PCM 上传
  ...
  async submit() {
    try {
      ...
      //    优化：在轮询过程中，一旦出现首个 generating 事件携带的视频地址，
      //    立即通过 onFirstVideo 下发给前端播放（边生成边播），不再等整段 complete，
      //    显著缩短数字人首帧延迟（对齐 SoulX 滑动窗口流式思路）。
      this.onMode?.('live'); // 先切 live，前端显示播放器容器，准备接收首个视频片段

      let earlyUrl = null;
      const finalPath = await this._pollEvent(eventId, (u) => {
        earlyUrl = u;
        this.sendControl?.({ type: 'video', url: u });
      });
      if (!finalPath) throw new Error('未获取到视频路径');
      ...
      const finalUrl = this._resolveVideoUrl(finalPath);
      if (!finalUrl) throw new Error('无法解析视频 URL');
      // 若完整视频路径与早先下发的首个片段不同（非连续直播清单），补发一次以校正播放源；
      // 若相同（连续增长的 m3u8），则不重复下发，前端持续播放同一清单。
      if (finalUrl !== earlyUrl) {
        this.sendControl?.({ type: 'video', url: finalUrl });
      }
    } catch (e) {
   ...
   * @param {(url:string)=>void} [onFirstVideo] 首个 generating 事件携带视频地址时的回调，
   *        用于尽早下发前端播放（边生成边播，降低首帧延迟）。
   * @returns {Promise<string|null>} 视频服务端路径
   */
  async _pollEvent(eventId, onFirstVideo) {
    ...
    try {
      ...
      let buf = '';
      let videoPath = null;
      let firstSent = false; // 首个 generating 视频地址是否已下发
      ...
      for (;;) {
        ...
        while ((idx = buf.indexOf('\n\n')) !== -1) {
          ...
          if (type === 'generating') {
            ...
            if (v) {
              videoPath = v;
              // 首个可用视频地址立即下发前端播放（边生成边播，缩短首帧延迟）
              if (onFirstVideo && !firstSent) {
                firstSent = true;
                try {
                  onFirstVideo(this._resolveVideoUrl(v));
                } catch (_) {
                  /* 解析失败忽略，等待 complete 兜底 */
                }
              }
            }
