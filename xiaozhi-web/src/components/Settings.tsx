...
export default function Settings({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const audioSyncPreferVideo = useChatStore((s) => s.audioSyncPreferVideo);
  const setAudioSyncPreferVideo = useChatStore((s) => s.setAudioSyncPreferVideo);
  const [portrait, setPortrait] = useState<string>('');
  const [fileName, setFileName] = useState<string>('');

  // 以本地 audioSync 实际值为准（含后端默认值），避免与 store 初始值不同步
  useEffect(() => {
    setAudioSyncPreferVideo(audioSync.getPreferVideoAudio());
  }, [setAudioSyncPreferVideo]);

  const onToggleAudioSync = (next: boolean) => {
    audioSync.configure({ preferVideoAudio: next, persist: true });
    setAudioSyncPreferVideo(next);
  };
  ...
  return (
    <ThemeProvider theme={darkTheme}>
      ...
      >
        ...
        {/* 音频优先级开关：以数字人视频为准 / 以原始 TTS 为准（本地持久化，无需重启） */}
        <Box className="mb-3 rounded bg-white/5 px-3 py-2">
          <FormControlLabel
            control={
              <Switch
                checked={audioSyncPreferVideo}
                onChange={(e) => onToggleAudioSync(e.target.checked)}
                color="primary"
              />
            }
            label={
              <Box>
                <Typography variant="body2" className="text-slate-100">
                  {audioSyncPreferVideo ? '以数字人视频音频为准' : '以原始语音（TTS）为准'}
                </Typography>
                <Typography variant="caption" className="block text-slate-400">
                  {audioSyncPreferVideo
                    ? '本地推理视频带声时仅播放视频音轨，不重复播报；视频无声音时自动回退 TTS。'
                    : '始终用原始语音播报，数字人视频强制静音，避免与视频音轨双声。'}
                </Typography>
              </Box>
            }
