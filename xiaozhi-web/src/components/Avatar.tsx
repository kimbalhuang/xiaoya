...
export default function Avatar({ variant = 'box' }: { variant?: 'box' | 'background' }) {
  ...
  // 音频优先级开关（audioSyncPreferVideo）：
  //   - true（以数字人为准，默认）：视频带声播放，TTS 缓冲不播（防重复），仅视频音轨发声；
  //   - false（以原始 TTS 为准）：视频强制静音，始终由 TTS 实时播报，避免与视频音轨双声。
  const tryPlay = (videoEl: HTMLVideoElement) => {
    // 以原始 TTS 为准：视频强制静音，仅作口型画面，声音完全交给 TTS
    if (!audioSync.getPreferVideoAudio()) {
      videoEl.muted = true;
      setVideoMuted(true);
      videoEl
        .play()
        .then(() => {
          audioSync.setVideoActive(false); // 视频无音轨 → TTS 实时播放
        })
        .catch(() => {});
      return;
    }
  ...
  useEffect(() => {
    ...
    const start = () => {
      if (disposed) return;
      if (Hls.isSupported()) {
        // 低延迟 / 高性能播放配置（不改变 UI 与基本逻辑，仅优化数字人视频起播与流畅度）：
        // - enableWorker：解封装/分片处理移入 worker 线程，避免阻塞 UI 主线程导致口型卡顿
        // - lowLatencyMode + liveSync*：优先贴近直播边缘，缩短起播与播放延迟
        // - maxBufferLength/backBufferLength：限制缓冲上下限，起播更快、内存更省
        // - abrEwmaDefaultEstimate：起播给定合理码率估计，避免首段低码率起播卡顿
        const hls = new Hls({
          enableWorker: true,
          lowLatencyMode: true,
          maxBufferLength: 12,
    ...
    return (
      <Box className={boxCls}>
        <video
          ...
          onEnded={() => {
            // 视频正常播放结束：
            // - 若视频当时带声起播：视频音轨已完整覆盖本段 TTS 音频，缓冲的 TTS 与之完全重复，
            //   必须丢弃（endVideo）而非补播，否则会出现"语音重复播放"。
            // - 若视频被静音（自动播放被浏览器拦截）：视频无音轨，需降级补播 TTS（setVideoActive(false)），
            //   保证用户仍听到完整回复。
            if (videoRef.current && videoRef.current.muted) {
              audioSync.setVideoActive(false); // 静音视频 → 补播 TTS 兜底
            } else {
              audioSync.endVideo(); // 带声视频已播完 → 丢弃缓冲 TTS，防重复
            }
            setMode('2d');
            setVideoUrl(null);
          }}
